		...
		$('body').delegate("li>span",'click dblclick contextmenu',function(event){
			switch(event.type)
			{
				case 'click':
					...
					break;
				case 'dblclick':
					$.post('ajax.php?c=arbolpredefinido&f=datosCuenta', 
						{
...
function sendObject(object, type)
{
	...
	{
		destination = "ajax.php?c=arbolpredefinido&f=" + type;
	}
	else
	{
		sending = false;
...
function guardar()
{
	...
	if(ejecuta)
	{
		...
		$.post('ajax.php?c=arbolpredefinido&f=guardaCuenta', 
		{
		...
		function(data)
		 {
			...
			if(parseInt(data) == 2)
		 		$("#mensajes").hide().fadeIn( 1000, "linear").css({"background-color":"red","color":"white"}).text("No se puede guardar la cuenta debido a que ya existe una cuenta de mayor para esta rama del arbolpredefinido.");
...
function eliminar()
{
	...
	if(sure)
	{
		$.post('ajax.php?c=arbolpredefinido&f=eliminarCuenta', 
		{
			idcuenta: $("#idcuenta").val()
		}, 
		function(data)
...
function cargaCuentas()
		{
			//INICIA OBTENCION DE arbolpredefinido CONTABLE
			$('#cont>ul').html('')
			...
			//TERMINA OBTENCION DE arbolpredefinido CONTABLE
		}
