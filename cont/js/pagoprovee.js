...
function guarda(){
		...
		if(fecha!=""){
			...
			$.post('index.php?c=CaptPolizas&f=guardaprove',{
				fecha:fecha,
				formapago:formapago,
					 ...
					 if(r[1]==1){ $("#load").hide();$("#agregaprevio").show();
						 ...
						 if (confirm("Poliza generada Correctamente!. \n Desea ver la poliza?")){
						 	$.post('ajax.php?c=CaptPolizas&f=consultaultima',{},
						 		function (idpoli){ 
						 			window.location= 'index.php?c=CaptPolizas&f=verpoliprove';
...
function actualizaCatalogos(opc){
	$.post('ajax.php?c=CaptPolizas&f=actualizaCatalogo',{
		opc:opc
		},function (request){
			if(opc == 1){
				if(request!=0){
					$("#banco").html(request).select2({width : "150px" });
				}else{
					$("#bancosno").text("Por favor seleccione una cuenta de bancos en el menu de configuracion o agregue cuentas al arbol ");
				}
			}
			else if(opc == 2){
				$("#proveedor").html(request).select2({width : "150px" });
			}
		});
}
