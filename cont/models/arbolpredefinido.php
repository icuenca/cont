...
class ArbolPredefinidoModel extends Connection
{
	...
	public function getAccounts()
	{
		...
		$accounts = "";
		//$qry = "SELECT *,(SELECT COUNT(*) FROM cont_movimientos m WHERE m.Cuenta = c.account_id AND m.Activo = 1) AS activity FROM cont_accounts_default c ORDER BY father_account_id,account_id;";
		$qry = "SELECT *,0 AS activity FROM cont_accounts_default c ORDER BY father_account_id, $TipoNiveles ;";
	...
	public function getMask()
	{
	 ...
	 public function guardar($numero,$nombre,$nombre_idioma,$subcuentade,$naturaleza,$moneda,$clasificacion,$digito,$estatus,$idcuenta)
	 {
	 	...
	 	if(intval($subcuentade))
	 	{
		 	...
		 	$myQuery = "SELECT* FROM cont_accounts_default WHERE account_id = $subcuentade AND removed=0 AND main_account != 3";
	 	...
	 	else//SI NO ES DE MAYOR O NO EXISTE ANCESTRO DE MAYOR
	 	{
	 		...
	 		//$myQuery = "SELECT account_code FROM cont_accounts_default WHERE father_account_id = $subcuentade AND removed = 0 ORDER BY account_id DESC LIMIT 1";
	 		$myQuery = "SELECT account_code, SUBSTRING_INDEX( account_code , '.', -1 ) AS orden FROM cont_accounts_default WHERE father_account_id =$subcuentade AND removed = 0 ORDER BY CAST(orden AS UNSIGNED) DESC LIMIT 1;";
			...
			if(intval($idcuenta))
		 	...
		 	$myQuery = "SELECT account_id FROM cont_accounts_default WHERE manual_code = '$numero' AND removed=0 $where";
			...
			//SI ES UNA ACTUALIZACION
		 	if(intval($idcuenta))
		 	{
		 		...
		 		$myQuery = "SELECT account_code,father_account_id,main_account,main_father FROM cont_accounts_default WHERE account_id = $idcuenta";
		 		...
		 		{
		 			$myQuery = "SELECT COUNT(account_id) AS num FROM cont_accounts_default WHERE removed = 0 AND account_code LIKE '$anterior_codigo.%' AND main_account = 1;";
		 		...
		 		{
		 			$myQuery = "SELECT COUNT(account_id) AS num FROM cont_accounts_default WHERE removed = 0 AND account_code LIKE '$anterior_codigo.%';";
		 		...
		 		$myQuery = "UPDATE cont_accounts_default SET ";
				...
				$this->transaccion('Modifica Cuenta',$myQuery);
					...
					//SI TIENE HIJOS MODIFICARA SU CUENTA DE MAYOR Y EL CODIGO
				 	$myQuery = "SELECT account_id,account_code,main_account FROM cont_accounts_default WHERE removed = 0 AND account_code LIKE '$anterior_codigo.%'";
		 	...
		 	else
		 	{
		 		...
		 		$myQuery = "INSERT INTO cont_accounts_default(account_id,account_code,manual_code,description,sec_desc,account_type,status,main_account,cash_flow,reg_date,currency_id,group_dig,id_sucursal,seg_neg_mov,affectable,mod_date,father_account_id,removable,account_nature,removed,main_father,cuentaoficial,nif) 
			 	...
			 	if(intval($clasificacion) == 1)
			 	{
			 		$myQuery2 = "UPDATE cont_accounts_default SET main_father = $ultima WHERE account_id = $ultima";
			 		$this->query($myQuery2);
			 	}
	 ...
	 {
	 	$myQuery = "SELECT a.*, (SELECT CONCAT('( ',manual_code,' ) ',description) FROM cont_accounts_default WHERE account_id=a.father_account_id) AS description_father FROM cont_accounts_default a WHERE a.account_id = $idCuenta";
	 ...
	 {
	 	//DATOS DE LA CUENTA
		$myQuery = "SELECT account_code,removable FROM cont_accounts_default WHERE account_id = $idcuenta";
		$datos = $this->query($myQuery);
		$datos = $datos->fetch_assoc();
		...
		if(intval($removable))
		{
			...
			$myQuery = "SELECT account_id FROM cont_accounts_default WHERE removed = 0 AND account_code LIKE '$account_code.%';";
			$hijos = $this->query($myQuery);
			...
			{
				$myQuery = "UPDATE cont_accounts_default SET removed = 1 WHERE account_id = $idcuenta $cuentasElim2";
	 ...
	 function corregirEstructura($vars)
	 {
	 	$codigo = '';

	 	//Guarda respaldo tabla
	 	$myQuery = "CREATE TABLE cont_accounts_default_resp LIKE cont_accounts_default;
	 				INSERT INTO cont_accounts_default_resp SELECT* FROM cont_accounts_default;";

	 	for($i=1; $i<=6; $i++)
	 	{
	 		switch($i)
	 		{
	 			case 1:$codigo = $vars['activo'];break;
	 			case 2:$codigo = $vars['pasivo'];break;
	 			case 3:$codigo = $vars['capital'];break;
	 			case 4:$codigo = $vars['ingreso'];break;
	 			case 5:$codigo = $vars['egreso'];break;
	 			case 6:$codigo = $vars['orden'];break;
	 		}
	 			$tipo = $i;
	 			if($i == 4)
	 				$tipo = 4.1;
	 			...
	 			$myQuery .= "SELECT @a$i := (SELECT a.account_code FROM cont_accounts_default a WHERE a.manual_code = '$codigo' AND removed = 0);
	 				UPDATE cont_accounts_default SET account_code = REPLACE(account_code,@a$i, '$tipo') WHERE account_code LIKE CONCAT(@a$i,'%');
	 				UPDATE cont_accounts_default SET account_type = ".intval($tipo)." WHERE account_code LIKE '$tipo%';
	 				UPDATE cont_accounts_default SET main_father = 0 WHERE account_code LIKE '$tipo%' AND main_father NOT IN (SELECT account_id FROM (SELECT a.account_id FROM cont_accounts_default a WHERE a.account_type = ".intval($tipo)." AND a.account_code LIKE '$tipo%' AND removed = 0 AND main_account = 1) AS c);";
	 	}
		...
		{
			$this->transaccion('Corregir Arbol Mal Formado',"Corrige Arbol");
			return 1;
		}
		else
		{
				$this->transaccion('Correccion Arbol fallo',"---");
			return 0;
		}

		/*CREATE TABLE cont_accounts_default_resp LIKE cont_accounts_default;

INSERT INTO cont_accounts_default_resp SELECT* FROM cont_accounts_default;

SELECT @a := (SELECT a.account_code FROM cont_accounts_default a WHERE a.manual_code = '000-0040' AND removed = 0);
SELECT @b:= (SELECT a.account_code FROM cont_accounts_default a WHERE a.manual_code = '000-0050' AND removed = 0);

UPDATE cont_accounts_default SET account_code = REPLACE(account_code,@a, '4.1') WHERE account_code LIKE CONCAT(@a,'%');
UPDATE cont_accounts_default SET account_code = REPLACE(account_code,@b, '4.2') WHERE account_code LIKE CONCAT(@b,'%');

UPDATE cont_accounts_default SET account_type = 4 WHERE account_code LIKE '4%';

UPDATE cont_accounts_default SET main_father = 0 WHERE account_code LIKE '4.1%' AND main_father NOT IN (SELECT account_id FROM (SELECT a.account_id FROM cont_accounts_default a WHERE a.account_type = 4 AND a.account_code LIKE '4.1%' AND removed = 0 AND main_account = 1) AS c);

UPDATE cont_accounts_default SET main_father = 0 WHERE account_code LIKE '4.2%' AND main_father NOT IN (SELECT account_id FROM (SELECT a.account_id FROM cont_accounts_default a WHERE a.account_type = 4 AND a.account_code LIKE '4.2%' AND removed = 0  AND main_account = 1) AS c);
