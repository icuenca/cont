													...
													<!--Separador**|**-->
															...
															while($info = $datos->fetch_object())
															{
																	if($anterior != $info->account_id)
																	{
																		...
																		if($cont!=0)
																		{
																			echo "<tr id='sub-$info->account_id' style='font-weight:bold;background-color:#f6f7f8;height:30px;' tipo='subtotal' align='right' cl='$anterior'></tr>";
																			echo "
																			<tr style='height:30px;' cl='$anterior'>
																				<td colspan='8'></td>
																				<td style='background-color:#dbdfe3'></td>
																				<td colspan='2' style='background-color:#dbdfe3';>
																					<strong>Saldo Despues:</strong>
																				</td>
																				<td style='background-color:#dbdfe3' cl='$anterior' tt='despues'> 
																					".number_format($this->ReportsModel->Saldos($anterior,$fecha_despues,'Despues',1,$_REQUEST['segmento'],$saldo,$_REQUEST['sucursal']),2)."</strong></td>
																			</tr>
																			<!--Separador**|**-->";
																			if($anteriorMayor != $info->main_father AND intval($_REQUEST['tipo']))
																			{
																				...
																				<tr style='height:30px;'>
																					...
																					<td></td>
																					<td style='background-color:#dbdfe3'></td>
																					<td style='background-color:#dbdfe3' colspan='2'><strong>Saldo Mayor Despues:</strong></td>
																		...
																		if($anteriorMayor != $info->main_father AND intval($_REQUEST['tipo']))
																		{
																			...
																			echo "
																			<tr class='otros' style='background-color:#edeff1;height:30px;' tipo='Mayores'>
																				<td colspan=2>Cuenta Mayor: <b>$NM[0]</b></td>
																				<td colspan=4><b>$NM[1]</b></td>
																				<td></td>
																				<td></td>
																				<td colspan='3'><strong>Saldo Mayor Antes: </strong></td>
																				<td>".number_format($this->ReportsModel->Saldos($info->IdMayor,$fecha_antes,'Antes',0,$_REQUEST['segmento'],$saldo,$_REQUEST['sucursal']),2)."</strong></td>
																			</tr>";
																		}
																		$saldos_antes_pinta = $this->ReportsModel->Saldos($info->account_id,$fecha_antes,'Antes',1,$_REQUEST['segmento'],$saldo,$_REQUEST['sucursal']);
																		echo "
																		<tr class='otros' cl='$info->account_id' style='background-color:#edeff1;height:30px;' tipo='Cuentas'>
																			<td colspan=3>Cuenta: <b>$info->Codigo_Cuenta</b></td>
																			<td colspan=5><b>$info->Descripcion_Cuenta</b></td>
																			<td></td>
																			<td colspan='2'><strong>Saldo Antes: </strong></td>
																			<td>
																				".number_format($saldos_antes_pinta,2)."</strong>
																			</td>
																		</tr>";
																		$sumasaldos = $saldos_antes_pinta;
																	}
																	if(strtotime($info->Fecha) >= strtotime($fecha_antes))
																	{
																		if(strpos($info->TipoMovto, 'Cargo M.E') === false && strpos($info->TipoMovto, 'Abono M.E') === false)
																		{
																			if(intval($info->account_nature) == 1)
																			{
																				$sumasaldos += $info->Abonos - $info->Cargos;
																			}
																			if(intval($info->account_nature) == 2)
																			{
																				$sumasaldos += $info->Cargos - $info->Abonos;
																			}
																			echo "
																			...
																			<td class='tdcontenido' title='Numero_Poliza'><a href='index.php?c=CaptPolizas&f=ModificarPoliza&id=$info->idpoliza' target='_blank'>#".$info->Numero_Poliza."</a> / Periodo ".$info->Periodo."</td>
																			...
																			<td class='tdcontenido' title='Sucursal'> ".$info->Sucursal."</td>
																			...
																			<td class='tdcontenido' title='Saldo' style='text-align:right;'>".number_format($sumasaldos,2)."</td></tr>";
																	...
																	if($cont == $cantidadMovs)
																	{
																		...
																		echo "
																		<tr style='height:30px;' cl='$info->account_id'>
																			<td colspan='7'></td>
																			<td style='background-color:#dbdfe3';></td>
																			<td colspan='3' style='background-color:#dbdfe3';color:white;>
																				<strong>Saldo Despues:</strong>
																			</td>
																			<td style='background-color:#dbdfe3';color:white;' cl='$info->account_id' tt='despues'> 
																				".number_format($this->ReportsModel->Saldos($anterior,$fecha_despues,'Despues',1,$_REQUEST['segmento'],$saldo,$_REQUEST['sucursal']),2)."</strong>
																			</td>
																		</tr>";
																		if(intval($_REQUEST['tipo']))
																		{
																			echo "
																			<tr style='height:30px;'>
																				<td colspan='7'></td><td style='background-color:#dbdfe3';></td>
																				<td style='background-color:#dbdfe3';color:white;' colspan='3'>
																					<strong>Saldo Mayor Despues:</strong>
																				</td>
																				<td style='background-color:#dbdfe3';color:white;'> 
																					".number_format($this->ReportsModel->Saldos($anteriorMayor,$fecha_despues,'Despues',0,$_REQUEST['segmento'],$saldo,$_REQUEST['sucursal']),2)."</strong>
																				</td>
																			</tr>";
																		}
																	}
														  ...
														  <tr id='xx' style='font-weight:bold;background-color:#d2d7dc;height:30px;' tipo='total' align='right'>
	...
	$(document).ready(function()
{
	...
	var total = 0;
	var subtotal = 0;
	for(var f = 10; f <= 11; f++)//Recorre cada columna
	{
		...
		$("#tabla_reporte tr").each(function(index)//Recorre cada fila
		{
			...
			if($(this).next().attr('tipo') == 'subtotal' && $(this).next().next().attr('tipo') != 'movimientos')//Si es un campo del tipo subtotal y el siguiente elemento no es de tipo numero entonces agrega elementos
				{
					if(f==10)//si se trata del primer barrido agraga td`s de relleno a la tabla
					{
						$(this).next().append("<td colspan='9'>Subtotal:</td>");
					}
